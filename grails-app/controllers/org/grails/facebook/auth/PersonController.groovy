...
class PersonController {
    ...
    def profile(Person personInstance) {
    ...
    private boolean isYourFriend(Person personInstance) {

        def currentUserId = springSecurityService.principal.id
        def Person currentUser = Person.get(currentUserId)
        return currentUser.friends?.contains(personInstance)
    ...
    def acceptFriendRequest (FriendRequest friendRequestId){
        ...
        try {
            ...
            flash.message = 'Friend request from '+ buddy +' was ACCEPTED.'
            redirect action: 'friends'
    ...
    def denyFriendRequest (FriendRequest friendRequestId){
        ...
        try {
            def buddy = friendRequest.from.realName
            friendRequest.delete(flush: true)
            flash.message = 'Friend request from '+ buddy +' was DENIED.'
            redirect action: 'friends'
